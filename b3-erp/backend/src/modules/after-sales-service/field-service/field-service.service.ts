...
export class FieldServiceService {
  ...
  createJob(createFieldServiceJobDto: CreateFieldServiceJobDto): FieldServiceJob {
    const job: FieldServiceJob = {
      ...
      repeatVisit: false,
      serviceReport: {
        problemStatement: createFieldServiceJobDto.problemDescription,
        diagnosis: '',
        workPerformed: '',
      },
      createdBy: 'system',
  ...
  submitServiceReport(
    ...
    const report: ServiceReport = {
      ...
      fieldServiceJobId: id,
      engineerId: job.assignedEngineerId,
      engineerName: job.assignedEngineerName,
      customerId: job.customerId,
      customerName: job.customerName,
      equipmentId: job.equipmentId || 'unknown',
      equipmentModel: job.equipmentModel || 'unknown',
      equipmentSerialNumber: job.equipmentSerialNumber || 'unknown',
      problemReported: job.problemDescription,
      symptomsObserved: job.symptomsReported || [],
      diagnosis: job.diagnosisNotes || '',
      rootCause: job.rootCause || '',
      workDescription: reportData.workPerformed,
      ...
      issueResolved: job.issueResolved,
      resolutionStatus: job.issueResolved ? 'resolved' : 'pending',
      followUpRequired: job.followUpRequired,
      customerPresent: job.customerPresent,
      customerSignature: reportData.customerSignature || '',
      signedDate: new Date(),
      qualityCheckCompleted: false,
      createdBy: 'system',
      serviceDate: job.actualStartTime || new Date(),
      ...reportData,
      partsReplaced: reportData.partsReplaced ? (reportData.partsReplaced as any[]).map(p => typeof p === 'string' ? { partNumber: p, partName: 'Unknown', quantity: 1 } : p) : [],
