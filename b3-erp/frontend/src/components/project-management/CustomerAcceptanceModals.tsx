  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl">
        ...
        <div className="p-6">
          <div className="space-y-4">
            <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 mb-4"><p className="text-sm text-emerald-800"><strong>Note:</strong> By signing this acceptance, you confirm that all deliverables meet the specified criteria and documentation is complete.</p></div>
            <div className="grid grid-cols-2 gap-4">
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 p-4 rounded-lg"><p className="text-sm text-blue-600 font-medium">Customer</p><p className="text-lg font-bold text-gray-900">{acceptance.customer}</p><p className="text-xs text-gray-600">{acceptance.customerContact}</p></div>
            <div className="bg-green-50 p-4 rounded-lg"><p className="text-sm text-green-600 font-medium">Acceptance Type</p><p className="text-lg font-bold text-gray-900">{acceptance.acceptanceType}</p><p className="text-xs text-gray-600">{acceptance.acceptanceDate}</p></div>
            <div className="bg-purple-50 p-4 rounded-lg"><p className="text-sm text-purple-600 font-medium">Status</p><p className="text-lg font-bold text-gray-900">{acceptance.overallStatus}</p><p className="text-xs text-gray-600">Phase: {acceptance.phase}</p></div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-6">
            <div className="border rounded-lg p-4"><h3 className="text-sm font-semibold text-gray-900 mb-2">Deliverables</h3><ul className="list-disc list-inside text-sm text-gray-700 space-y-1">{acceptance.deliverables.map((d: string, i: number) => (<li key={i}>{d}</li>))}</ul></div>
            <div className="border rounded-lg p-4"><h3 className="text-sm font-semibold text-gray-900 mb-2">Warranty & Support</h3><div className="text-sm space-y-1"><p><strong>Warranty:</strong> {acceptance.warrantyPeriod} (from {acceptance.warrantyStartDate})</p><p><strong>AMC:</strong> {acceptance.amcOffered ? `${acceptance.amcDuration} offered` : 'Not offered'}</p><p><strong>Training:</strong> {acceptance.trainingCompleted ? 'Completed' : 'Pending'}</p></div></div>
          </div>
          {acceptance.remarks && (<div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4"><p className="text-sm font-semibold text-gray-900 mb-1">Remarks:</p><p className="text-sm text-gray-700">{acceptance.remarks}</p></div>)}
          {acceptance.signedBy && (<div className="mt-4 border-t pt-4"><div className="grid grid-cols-2 gap-4 text-sm"><div><p className="text-gray-600">Signed by:</p><p className="font-semibold text-gray-900">{acceptance.signedBy} ({acceptance.signedByDesignation})</p><p className="text-xs text-gray-500">{acceptance.signedDate}</p></div><div><p className="text-gray-600">Witnessed by:</p><p className="font-semibold text-gray-900">{acceptance.witnessedBy}</p></div></div></div>)}
        </div>
        <div className="bg-gray-50 px-6 py-4 flex justify-end space-x-3 border-t">
