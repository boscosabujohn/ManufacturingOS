...
const SourcingEvents: React.FC = () => {
  ...
  const handleCreateEvent = () => {
    ...
    alert('Create New Sourcing Event - This would open a wizard to configure RFQ, RFP, RFx, or Reverse Auction with evaluation criteria and timeline.');
  };

  const handleRefresh = () => {
    console.log('Refreshing sourcing events...');
    alert('Refreshing Sourcing Events - Updating event status, bid counts, and auction prices.');
  };

  const handleSettings = () => {
    console.log('Opening sourcing settings...');
    alert('Sourcing Events Settings - Configure event timelines, evaluation weights, auction settings, and notification preferences.');
  };

  const handleExportEvents = () => {
    console.log('Exporting sourcing events report...');
    alert('Export Sourcing Events Report - This would generate comprehensive event summary, bid analysis, and savings reports.');
  };

  const handleViewEventDetails = (event: SourcingEvent) => {
    console.log('Viewing event details:', event.id);
    alert(`Event Details: ${event.eventNumber} - ${event.title}. Type: ${event.type.toUpperCase()}, Status: ${event.status.toUpperCase()}, Responses: ${event.responsesReceived}/${event.participantsInvited}`);
  };
  ...
  const handleEditEvent = (event: SourcingEvent) => {
    ...
    if (event.status === 'awarded' || event.status === 'cancelled') {
      alert(`Cannot Edit Event: Event ${event.eventNumber} is ${event.status} and cannot be edited.`);
      return;
    }
    alert(`Edit Event: ${event.eventNumber} - This would open an editing form for event details and specifications.`);
  };

  const handlePublishEvent = (event: SourcingEvent) => {
    console.log('Publishing event:', event.id);
    if (event.status !== 'draft') {
      alert(`Event ${event.eventNumber} is already ${event.status}.`);
      return;
    }
    alert(`Publish Event: ${event.eventNumber} - This would publish the event to suppliers and send invitations.`);
  };
  ...
  const handleCloseEvent = (event: SourcingEvent) => {
    ...
    alert(`Close Event: ${event.eventNumber} - This would close bidding and move to evaluation phase.`);
  };

  const handleAwardEvent = (event: SourcingEvent) => {
    console.log('Awarding event:', event.id);
    if (event.status !== 'evaluation') {
      alert(`Event ${event.eventNumber} is not ready for award (current status: ${event.status}).`);
      return;
    }
    alert(`Award Event: ${event.eventNumber} - This would open the award recommendation and approval workflow.`);
  };

  const handleCancelEvent = (event: SourcingEvent) => {
    console.log('Cancelling event:', event.id);
    if (event.status === 'awarded') {
      alert('Cannot Cancel Awarded Event - Event has been awarded and cannot be cancelled.');
      return;
    }
    alert(`Cancel Event: ${event.eventNumber} - This would cancel the event and notify all participants.`);
  };

  const handleViewBid = (bid: BidResponse, event?: SourcingEvent) => {
    console.log('Viewing bid details:', bid.id);
    alert(`Bid Details: ${bid.supplier} - Amount: $${(bid.bidAmount / 1000000).toFixed(2)}M, Technical: ${bid.technicalScore}/100, Overall: ${bid.overallScore}/100, Status: ${bid.status.toUpperCase()}`);
  };

  const handleCompareBids = (event: SourcingEvent) => {
    console.log('Comparing bids for event:', event.id);
    if (event.responsesReceived === 0) {
      alert('No bids to compare for this event.');
      return;
    }
    alert(`Bid Comparison: ${event.eventNumber} - This would show a side-by-side comparison of ${event.responsesReceived} bids.`);
  };

  const handleMonitorAuction = (auction: AuctionEvent) => {
    console.log('Monitoring auction:', auction.id);
    alert(`Auction Monitor: ${auction.eventNumber} - Status: ${auction.status.toUpperCase()}, Current: $${(auction.currentPrice / 1000).toFixed(0)}K, Savings: $${(auction.savings / 1000).toFixed(0)}K`);
  };
