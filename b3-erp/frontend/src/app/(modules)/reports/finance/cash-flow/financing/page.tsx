...
function FinancingActivitiesContent() {
    const router = useRouter();

    const transactions = [
        { id: 'FIN-001', date: '2025-01-05', description: 'Loan Repayment - HDFC', category: 'Loan Repayment', amount: -50000 },
    ];

    return (
        <ReportDetailPage
            title="Financing Activities"
            description="Cash flow from activities that result in changes in the size and composition of contributed equity and borrowings"
            breadcrumbs={[
        ...
        >
            <Card>
                <CardHeader>
                    <CardTitle>Financing Transactions</CardTitle>
                </CardHeader>
...
export default function FinancingActivitiesPage() {
    return (
        <Suspense fallback={<div>Loading...</div>}>
            <FinancingActivitiesContent />
        </Suspense>
    );
}
