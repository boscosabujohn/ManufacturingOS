  ...
  const handleFitmentCheckToggle = (id: string) => {
    setJobs(jobs.map(job => {
      if (job.id === id) {
        ...
        if (newValue) {
          toast({ title: 'Fitment Verified', description: `Fitment checked for ${job.shutterName}. Ready for trial wall.` });
        }
    ...
    return (
      <div className="w-full py-2 space-y-6 px-3">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Select Project for Shutter Work</h1>
            <p className="text-sm text-gray-500 font-medium">Step 5.6: Manage shutter manufacturing and fitment checks</p>
          </div>
        </div>
    ...
    <div className="w-full py-2 space-y-4 px-3 font-sans">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => router.push('/project-management/production/trial-wall')} size="sm">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20">
          <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-primary"></div>
          <p className="mt-4 text-gray-500 font-medium">Loading shutter jobs...</p>
        </div>
      ) : (
        <>
          <Card className="border-l-4 border-l-blue-500 bg-blue-50 border-none shadow-sm">
            <CardContent className="pt-4 flex items-start gap-2">
              <AlertTriangle className="h-5 w-5 text-blue-600 mt-0.5" />
              <div>
                <h3 className="font-bold text-blue-700 text-sm">HANDLING INSTRUCTION</h3>
                <p className="text-xs text-blue-600">Handle glass and acrylic shutters with care. Ensure protective film is intact.</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-none shadow-sm ring-1 ring-gray-200 overflow-hidden">
            <CardHeader className="bg-gray-50/50 border-b py-4">
              <div className="flex items-center gap-3">
                ...
                <CardTitle className="text-lg font-bold tracking-tight">Shutter Job Queue</CardTitle>
              </div>
            </CardHeader>
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-50/30 text-[10px] font-bold uppercase tracking-widest text-gray-500">
                  <tr>
                    ...
                    <th className="p-4">Shutter Name</th>
                    <th className="p-4">Type</th>
                    <th className="p-4">Dimensions</th>
                    <th className="p-4">Qty</th>
                    <th className="p-4">Fitment Check</th>
                ...
                <tbody>
                  {jobs.map((job) => (
                    <tr key={job.id} className="border-t hover:bg-blue-50/20 transition-colors">
                      ...
                      <td className="p-4 font-medium">{job.shutterName}</td>
                      <td className="p-4"><Badge variant="outline" className="text-[10px] font-bold">{job.type}</Badge></td>
                      <td className="p-4 font-medium">{job.dimensions}</td>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            id={`qc-${job.id}`}
                          />
                          <label htmlFor={`qc-${job.id}`} className={`text-xs font-bold ${job.fitmentCheck ? 'text-green-600' : 'text-gray-500'}`}>
